...
const prodBundler = () => {
  return {
    module: {
      rules: [svgLoader(), cssLoader(), ...typescriptLoader()],
    },
  };
};
 ...
 */
const prodConfig = () => {
  const baseProdConfig = merge(sharedConfig(), sharedProdConfig(), prodBundler(), {
    mode: 'production',
  });
  ...
  return [BrowserConfig, EsmConfig, CjsConfig];
};
/**
 * @param {object} config
...
const devConfig = ({ env }) => {
  ...
  const sharedDevConfig = () => {
    return {
      module: {
        rules: [svgLoader(), cssLoader(), ...typescriptDevLoader()],
      },
      plugins: [
      ...
      resolve: {
        alias: {
          '@tern-secure/shared': path.resolve(__dirname, '../shared/src'),
        },
      },
      devServer: {
        allowedHosts: ['all'],
  ...
  const configToMerge = merge(entry('ternsecure.browser'), sharedConfig(), sharedDevConfig(), {
    mode: 'development',
  });

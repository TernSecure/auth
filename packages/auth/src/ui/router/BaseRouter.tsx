...
import { RouteContext } from './RouterCtx';
...
export const BaseRouter = ({
  ...
  urlStateParam,
  children,
}: BaseRouterProps): React.JSX.Element => {
  // Disabling is acceptable since this is a Router component

  const { navigate: ternSecureNavigate } = useTernSecure();
  ...
  const baseNavigate = async (toURL: URL | undefined): Promise<unknown> => {
    ...
    if (isOutsideOfUIComponent || isCrossOrigin) {
      const res = await ternSecureNavigate(toURL.href);
